// This assembler is used by Minimal UART 2.0 - ... and Minimal 64
// mnemonics starting with 'F' are interpreted as fast jumps (only used by Minimal UART Ultra)
...
/*
	...
	variants of the Minimal 64 and Minimal UART Ultra CPU,
	either integrated into an emulator or as an executable
...
char opCode(const std::string& src, int p, int len)
{
...
void Assembler(const std::string& src, const std::vector<std::string>& mnemonics, std::stringstream& hexout, std::stringstream& errors, bool dosym, std::string symtag)
{
	// PASS 1:
	// - Every #org sets pc.
	// - Every element increases pc by its bytesize
	// - Calculates addresses of label definition
	...
	while (findelem(src, ep) != 0)
	{
		...
		else																										    // SIMPLE EXPRESSSION
		{
			...
			else if (opCode(src, ep, elen) != -1) 										// mnemonic
			{
				pc++;
		...
		while (findelem(src, ep) != 0)
		{
			...
			else // evaluate the element size
			{
				...
				else if (opCode(src, ep, elen) != -1)						// mnemonic
				{
					...
					if (isemit) HEX.Emit(opCode(src, ep, elen));
				}
...
// This assembler reads an assembly language source file, translates the mnemonics into machine opcodes,
// and emits the code in Intel Hex format. The program is structured into two main passes: 1. Parse
// the source code and store label definitions and their program counter (PC) values. 2. Emit the
...
// definition = label, ':'
// string     = "'", { char - ( '\n' | '\0' | "'" ) }, "'"
// program    = [gap], [ element, { gap, element } ], [gap], '\0'
