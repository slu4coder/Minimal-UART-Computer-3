/*
  -------------------------------------------------------
  Minimal UART 3 Assembler by Carsten Herting (slu4) 2026
  -------------------------------------------------------
  ...
  while (findelem(src, ep) != 0)
  {
    ...
    else																										    // SIMPLE EXPRESSSION
    {
      ...
      if (src[ep] == '0' && src[ep+1] == 'x') 									// hex word or byte
      {
        ...
        if (k - ep > 4) pc += 2; else pc++;											// ... elen > 4 would miscount byte expr as 2
      }
      else if (src[ep] >= '0' && src[ep] <= '9') pc++; // plain number (8-bit only) 
      else if (opCode(src, ep, elen) != -1) pc++; 							// mnemonic?
